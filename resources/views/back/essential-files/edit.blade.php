...
@section('toolbar')
    <div class="d-flex gap-2">
        ...
        <a href="{{ $essentialFile->download_url }}" class="btn btn-info btn-sm" target="_blank">
...
@section('content')
    <form action="{{ route('admin.essential-files.update', $essentialFile) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                    ...
                    <div class="admin-card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control admin-form-control @error('title') is-invalid @enderror"
                                   value="{{ old('title', $essentialFile->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-3">
                            <label class="admin-form-label">Current Document</label>
                            <div class="alert alert-info">
                                <div class="d-flex align-items-center">
                                    <i class="{{ $essentialFile->file_icon }} me-3 fa-2x"></i>
                                    <div>
                                        <strong>{{ $essentialFile->title }}</strong>
                                        <br><small class="text-muted">{{ strtoupper($essentialFile->file_type) }} • {{ $essentialFile->formatted_file_size }}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Document Upload -->
                        <div class="mb-3">
                            <label class="admin-form-label">Replace Document (Optional)</label>
                            ...
                            <small class="text-muted d-block mt-2">Leave empty to keep current document. Accepted formats: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, TXT. Maximum 10MB.</small>
                            @error('document')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="admin-card-body">
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input type="checkbox" name="is_active" class="form-check-input" id="is_active"
                                       {{ old('is_active', $essentialFile->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="order" class="form-control admin-form-control @error('order') is-invalid @enderror"
                                   value="{{ old('order', $essentialFile->order) }}" min="0" required>
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-admin-primary">
                                <i class="fas fa-save me-2"></i>Update Essential File
                            </button>
                            <a href="{{ route('admin.essential-files.show', $essentialFile) }}" class="btn btn-admin-outline">
